...
let gulp = require ('gulp'),
    ...
    gulp.task('scss', function(){ //делаем из своего scss-кода css для браузера
     ...
     .pipe(prefixer({ //добавляем вендорные префиксы
       overrideBrowserslist: ['last 22 versions'], //последние 8 версий, но можно донастроить на большее или меньшее значение
       browsers: [
        "Android >= 4",
        "Chrome >= 20",
        "Firefox >= 24",
    ...
    gulp.task('images', function(){ //пережимаем изображения и складываем их в директорию build
      ...
      //     loops: 1, //количество прогонок изображения
    ...
    gulp.task('browser-sync', function() { //настройки лайв-сервера
      browserSync.init({
          ...
          //tunnel: "z-union", //тут можно прописать название проекта и дать доступ к нему через интернет. Работает нестабильно, запускается через раз. Не рекомендуется включать без необходимости.
